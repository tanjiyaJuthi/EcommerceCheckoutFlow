...
const UpdateRole = () => {
  ...
  const {data: roleData, isLoading} = useQuery({
    queryKey: ['get_update_role'],
    queryFn: async() =>{
      const res = await axiosSecure(`/api/roles-edit/${id}`)
      return res.data
    } 
  })
  ...
  useEffect(() => {
    if (roleData) {
      const initialPermissions = roleData.rolePermissions
  ...
  const groupedRoleAccess = roleAccess
    ...
    const handleSubmit = async() => {
        ...
        try {
            const res = await axiosSecure.put(`/api/roles/${id}`, {
              name: roleName,
    ...
    <div className="text-black mx-2 my-4">
        <section>
          ...
          <input
            ...
            name="roleName"
            defaultValue={roleData?.singleRoleData?.name}
            ...
            <div className="p-3 grid grid-cols-2 gap-10">
              {permissions.map((permission) => (
                <label key={permission.name} className="flex items-start">
                  ...
                  className="w-4"
                    type="checkbox"
                    // defaultChecked={checkedGroupRoleAccess.includes}
